...
| Stage 2 circular ERP mean IoU | 约 `0.194` | learned presence/quality 已加载，但旧 Beta calibration 严重压低分数 |
| Stage 2 valid rate | `0` | 证明需要重新校准，不证明 checkpoint 加载失败 |

这些数据不是发布结论，也不足以决定 Stage 3 阈值。Stage 3 结束前不得改动当前 `candidateMinScore`、`fusionSourceMinConfidence`、外观/运动组合权重和旧 Beta 参数。
...
| E01 | Stage 3 串行，旧校准 | validation | 模型原始定位能力 | checkpoint 可用 |
...
3. 运行 E00/E01，确认 Stage 3 原始定位和旧校准影响。
...
这一路径优先解决当前最明确的问题，即新模型分数与旧 Beta calibration 不匹配；随后用 R2 覆盖、Fusor 框形状和 Geometry 分层诊断提高 IoU；最后再通过 FP16、数据搬运、batch 合并和 TensorRT 降低延迟。
...
- Stage 3 checkpoint 尚未在本分支进行 calibration、validation A/B 或 holdout 验收。
