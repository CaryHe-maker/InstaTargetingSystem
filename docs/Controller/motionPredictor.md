...
实现位于 `controller/motion_estimator.py::SphericalMotionEstimator`。目标是输出下一帧搜索中心、目标角尺寸和不确定度。

## 样本选择

历史是长度为 `windowLength` 的可靠测量 deque。预测帧和未接受的弱候选不会加入历史。每个样本保存球面中心、时间戳、角尺寸和置信度。
...
水平/垂直角尺寸在 log 空间分别做线性拟合，再把 log rate 裁剪到 `maxLogScaleRatePerSec`。

## 协方差与可靠性

角不确定度由拟合残差、`processNoiseRadPerSec × dt` 和有限样本项组成；预测置信度随时间指数衰减。`MotionPrediction` 同时输出 2×2 中心协方差和 2×2 log 尺度协方差。当前第一版使用由标量不确定度构成的对角协方差，接口允许后续 Kalman 实现提供非对角项。
...
控制器把运动启动与公开测量提交解耦：初始化后的首个非空候选即使没有通过状态提交，也只会向运动窗口写入一次有界低置信度启动样本，不更新公开 bbox/BFoV。正常 backend 每轮产生候选时，第三个输入帧开始前窗口已有两个时间点，速度和运动残差评分可以工作；后续弱候选不会持续写入并污染窗口。RecoveryPlanner 用角不确定度扩展运动回退包络；正常线程中 TRACKING 的搜索 ViewSpec 按预测目标角尺寸动态使用 30°–120° FOV，UNCERTAIN 固定使用 120°。保留的显式 LOST 组件也固定使用 120°。
...
中心/尺度协方差残差函数保留为离线诊断能力，但不作为 Runtime 的 `effectiveMotion` 或 SingleScore 输入。
