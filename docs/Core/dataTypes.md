...
`FramePacket` 是一帧的唯一跨模块容器，包含序列 ID、帧号、单调时间戳、ERP RGB 和可选 `SegmentationPlane`。分割掩码若存在，必须与 RGB 空间对齐。
...
- `LocalView`：Geometry 实际裁出的 RGB 和原始 ViewSpec。
- `LocalObservation`：Tracker 返回局部 bbox、模型/外观分数和原始 backend `fusedScore`；Runtime 另附不覆盖原值的 `appearanceProbability`。
...
`MotionState3D` 除中心和速度外，还携带目标角尺寸、中心/尺度协方差和 reliability，使 Runtime 无需依赖 Controller 内部类型即可计算候选运动残差。
...
`TrackResult` 是对外结果，包含 ERP bbox、BFoV、置信度、公开状态、valid 和结果来源。`valid=False` 不等于没有 bbox；它可能携带运动预测框，但该框不得进入可靠测量历史。
