...
实现位于 `controller/recovery_planner.py`。LOST 的 cubemap 使用
...
也使用上一帧已提交 BFoV，不回退到固定四角布局。UNCERTAIN 的两轮 Type1 将 FOV 下限
提升到 120°，因此每张视图均为 120°×120°；LOST 保持 cubemap 加 Type1 的单轮恢复布局。
...
LOST 首轮一次生成以预测中心为 front 的 6 张 cubemap，以及同一预测中心的 Type1 四角 4 张视图。这样 10 张视图可以在同一轮统一交给 Fusor，不需要追加第二轮。
