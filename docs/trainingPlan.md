...
> 本文是后续实现训练、模型改造和推理优化的执行规范。除非新的实验记录明确证明某项假设错误，否则后续代码修改应以本文为准。
...
本文针对当前“RGB ERP -> 局部透视视图 -> HiT-Small -> 球面/ERP 回投”的跟踪路线。当前系统输出的是球面方向、角尺寸和 BFoV，不是带深度的公制 XYZ/6DoF 三维框；如果以后要求真实公制三维定位，需要额外的深度、双目或多相机几何方案。

## 2. 当前实现的事实与限制

1. `src/instatarget/training/losses.py`、`training/train_backend.py` 和 `configs/train_backend.yaml` 仍是 TODO，当前项目没有可执行训练循环。
2. `AirSim360TrainingDataset` 目前返回 ERP RGB、实例框和 visible 标志，尚未生成与生产 Geometry 一致的 template/search 局部视图对。
3. `__getitem__` 会从视频头部顺序读到目标帧，不适合作为高频随机训练采样；必须先建立 manifest 和帧索引。
4. 当前 HiT 输入为 template `128x128`、search `256x256`，模型输出由 corner head 产生 bbox。
5. 当前所谓置信度来自 corner heatmap 熵的人工变换，不是训练得到的目标存在概率。
6. 当前 Tracker 固定使用第 0 帧 anchor；每个正常帧固定执行 TRACKING/UNCERTAIN 的 4+4 个局部视图，第二轮依赖第一轮结果。
7. 当前 `TemplateCache` 缓存的是预处理后的模板张量，不是完整 HiT backbone 特征；不能未经验证地宣称模板已被完整缓存。
8. `onnx_backend.py` 和 `tensorrt_backend.py` 尚未形成生产路径；官方 HiT README 中的单视图 FPS 不能直接代表本系统的 ERP 多视图端到端 FPS。
9. 严格配置 schema 会拒绝未同步的字段；新增训练字段必须放入独立 `TrainingConfig`，不能塞入 `AppConfig` 的推理配置。
10. `src/instatarget/vendor/hit/lib` 被通用 `lib/` 忽略规则影响，且 `pyproject.toml` 只声明了 vendor 的 LICENSE/config 包数据。实现前必须解决源代码可追踪、可安装和可发布问题。
...
9. `controller/fused_score.py`：新模型上线后重新拟合外观校准；保留旧分数作为兼容/诊断，不覆盖新字段。
...
12. `tests/`：覆盖 loss mask、负样本、输出顺序、checkpoint 加载、冻结参数集合、概率范围、跨缝/极点局部 crop、事务两轮和旧权重回归。
...
每个候选 checkpoint 必须在同一 final holdout 上报告：
...
- 不要把运动预测写入训练真值或运动历史。
- 新模型必须能加载旧 checkpoint，旧模型也必须能继续运行，便于 A/B 和回滚。
