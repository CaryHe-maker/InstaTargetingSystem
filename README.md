...
GitHub 已包含小于 100 MB 的压缩 Stage 3 checkpoint 及其哈希绑定校准文件；PostTrainV2.4 保持 V2.3 的运行逻辑不变，仅更新交付版本标识。国内构建服务器 clone 后可直接执行 `python docker/verify_submission.py` 和 `docker build`。提交镜像固定基于 `pytorch/pytorch:2.11.0-cuda12.8-cudnn9-devel`，包含 RTX 5090 D v2 所需的 `sm_120` 支持；当前实现为 7 个 RootFS layer，低于最多 10 层的限制。具体流程见 [提交容器运行环境](docs/Competition/containerRuntime.md)。
