...
> 本文是 Stage 3 训练期间及训练完成后的推理流水线预适配规范。它描述可以提前完成的代码边界、必须保持不变的语义、暂定参数和后续真实数据适配流程。
>
...
- 当前 `singleScore = 0.70 * appearanceProbability + 0.30 * effectiveMotionProbability`。
- 当前 `appearanceProbability` 使用代码中的旧 Beta Calibration；Stage 3 新模型完成后必须在独立 calibration split 重新拟合。
...
- Stage 2 权重已正确加载 learned presence/quality 字段，但旧校准把新模型分数严重压低。
- Stage 2 同一序列 circular ERP mean IoU 约为 `0.194`，`validRate = 0`；这不是模型 checkpoint 加载失败，而是校准尚未重新拟合的证据。
- 因此，当前不能用 Stage 2 的最终 Controller 分数分布直接决定流水线回退阈值。
...
- 在未校准前修改旧 Beta 参数或通过提高阈值掩盖分数分布问题。
...
- legacy checkpoint 仍可运行。
- Stage 2/Stage 3 checkpoint 可运行。
